...
- On the opener, signature selection lands better when the title cards read like a tiny reward draft: `label + start core + one bias line` only. If the title screen regrows tag chips, seed-core inventory, perk paragraphs, or launch brief copy, the opening has slipped back into admin mode before the player has moved.
