...
assert.equal(game.WAVE_CONFIG[10].activeCap, 32);
assert.equal(game.WAVE_CONFIG[11].activeCap, 39);
...
assert.equal(game.WAVE_CONFIG[10].arena.width, 1640);
assert.ok(game.WAVE_CONFIG[8].hazard.interval > game.WAVE_CONFIG[10].hazard.interval);
...
assert.equal(lateProofSummary.label, "Proof Rung");
assert.ok(lateProofSummary.detail.includes("pursuit"));
...
assert.equal(game.WAVE_CONFIG[10].arena.height, 940);
...
assert.ok(sharedLateWaveTen.activeCap < sharedLateWaveEleven.activeCap);
