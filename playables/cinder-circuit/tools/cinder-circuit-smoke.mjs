...
assert.equal(game.POST_CAPSTONE_WAVE_COUNT, 5);
...
assert.ok(mirrorWaveNine.mix.shrike > mirrorWaveNine.mix.scuttler);
assert.ok(mirrorWaveTen.hazard.relayRange > game.WAVE_CONFIG[9].hazard.relayRange);
assert.ok(mirrorWaveEleven.mix.mortar >= mirrorWaveEleven.mix.shrike);
...
assert.equal(afterburnTransitionRun.postCapstone.total, 5);
...
assert.equal(afterburnThree.hazard.type, "relay");
assert.equal(afterburnFour.hazard.type, "drift");
...
assert.ok(afterburnFive.activeCap > afterburnThree.activeCap);
assert.equal(afterburnFive.completesRun, true);
...
assert.ok(finalForgeChoices[2].finalePreview.tempo.includes("5연전"));
...
assert.ok(finalAfterburnWave.label.includes("Afterburn V"));
