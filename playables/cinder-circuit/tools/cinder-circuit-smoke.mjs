...
assert.equal(game.WAVE_CONFIG[10].activeCap, 30);
assert.equal(game.WAVE_CONFIG[11].activeCap, 35);
...
assert.equal(game.WAVE_CONFIG[10].arena.width, 1700);
assert.equal(game.WAVE_CONFIG[11].arena.width, 1560);
...
assert.equal(game.WAVE_CONFIG[9].hazard.type, "caravan");
assert.equal(game.WAVE_CONFIG[9].pressureFamily, "caravan");
assert.equal(game.WAVE_CONFIG[10].pressureFamily, "crossfire");
assert.equal(game.WAVE_CONFIG[11].pressureFamily, "breach");
...
assert.equal(latePayoffSummary.label, "Payoff Band");
assert.ok(latePayoffSummary.detail.includes("late form"));
assert.equal(lateProofSummary.label, "Victory Lap");
assert.ok(lateProofSummary.detail.includes("열린 lane"));
assert.equal(lateFinaleSummary.label, "Finale");
assert.ok(lateFinaleSummary.detail.includes("마지막 breach"));
...
assert.ok(game.WAVE_CONFIG[9].hazard.count <= game.WAVE_CONFIG[8].hazard.count);
...
assert.ok(game.WAVE_CONFIG[9].hazard.salvageScrap > 0);
assert.ok(game.WAVE_CONFIG[9].hazard.salvageBurstCount > 0);
...
assert.equal(game.WAVE_CONFIG[10].arena.height, 980);
...
assert.equal(lockgridPayoff.bandId, "lockgrid_gallery");
assert.equal(lockgridEscalation.bandId, "crownsplit_run");
assert.equal(lockgridEscalation.pressureFamily, "caravan");
...
assert.equal(lockgridPayoff.hazard.type, null);
assert.equal(lockgridEscalation.hazard.type, "caravan");
assert.ok(lockgridEscalation.hazard.count <= lockgridPayoff.hazard.count);
...
assert.equal(lateRouteProofBeat.label, "Victory Lap");
assert.ok(lateRouteProofBeat.detail.includes("Starforge Gallery"));
...
assert.equal(lateRouteProofWindow.label, "Victory Lap");
assert.ok(lateRouteProofWindow.detail.includes("Starforge Gallery"));
...
assert.equal(crownWindowPayoff.bandId, "starforge_gallery");
...
assert.equal(bastionWaveTen.arena.width, 1840);
assert.equal(artilleryWaveTen.arena.width, 1900);
assert.equal(mirrorWaveEleven.arena.width, 1940);
assert.equal(mirrorWaveTwelve.arena.height, 1030);
assert.equal(bastionWaveEleven.arena.width, 1780);
assert.equal(bastionWaveTwelve.arena.height, 1000);
assert.equal(artilleryWaveEleven.arena.width, 1860);
...
assert.equal(artilleryWaveEleven.hazard.count, 3);
...
assert.equal(sharedLateWaveTen.label, "Wave 10 · Crownsplit Run");
assert.equal(sharedLateWaveTen.pressureFamily, "caravan");
assert.equal(sharedLateWaveTen.hazard.type, "caravan");
assert.equal(sharedLateWaveEleven.label, "Wave 11 · Starforge Gallery");
assert.equal(sharedLateWaveEleven.hazard.type, "drift");
assert.equal(sharedLateWaveTwelve.label, "Wave 12 · Cinder Crown");
assert.equal(sharedLateWaveTwelve.hazard.type, "relay");
assert.ok(sharedLateWaveTwelve.hazard.count <= 3);
...
assert.ok(sharedLateWaveTwelve.activeCap <= 35);
assert.ok(sharedLateWaveTen.hazard.count < sharedLateWaveTwelve.hazard.count);
assert.ok(sharedLateWaveTen.hazard.salvageScrap > 0);
