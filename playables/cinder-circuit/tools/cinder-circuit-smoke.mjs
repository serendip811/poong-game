...
assert.equal(
  JSON.stringify(game.getVisibleSupportOfferSystemIds(wave8SupportlessBuild, 8)),
  JSON.stringify(["ember_ring"])
);
assert.equal(
  game
...
assert.equal(game.WAVE_CONFIG[11].activeCap, 30);
...
assert.equal(game.WAVE_CONFIG[11].arena.width, 2040);
assert.ok(game.WAVE_CONFIG[10].arena.width > game.WAVE_CONFIG[9].arena.width);
assert.ok(game.WAVE_CONFIG[11].arena.width > game.WAVE_CONFIG[7].arena.width);
...
assert.equal(game.WAVE_CONFIG[11].hazard.label, "Final Stand Drift");
...
assert.equal(
  ...
  JSON.stringify(["START", "THESIS", "ROUTE", "LAP"])
);
...
assert.equal(midrunContractSummary.leadLabel, "다음 경로");
assert.equal(midrunContractSummary.leadValue, "Wave 5 진로 선택");
const midrunLadderSteps = game.getShippingLadderSteps(
  roadmapBuild,
  game.computeWeaponStats(roadmapBuild),
  5
);
assert.match(midrunLadderSteps[0].detail, /Wave 5 경로 선택/);
assert.match(midrunLadderSteps[1].detail, /Wave 5 경로 선택/);
assert.equal(midrunLadderSteps[2].label, "ROUTE");
assert.equal(midrunLadderSteps[2].title, "Wave 5 진로 선택");
...
assert.match(doctrinePromiseChoice.description, /body plan만 먼저 잠근다/);
assert.match(doctrinePromiseChoice.description, /첫 visible hardware는 Wave 8/);
...
assert.equal(wave6Choices.length, 3);
const chassisOnlyChoice = wave6Choices.find((choice) => choice.chassisId === "bulwark_treads");
assert.ok(chassisOnlyChoice);
const greedFrameChoice = wave6Choices.find((choice) => choice.action === "field_greed");
assert.ok(greedFrameChoice?.wave6GreedFrame);
...
assert.equal(wave6PreviewRows[1]?.value, "hold, dive, exit 리듬 교체");
assert.equal(wave6PreviewRows.length, 2);
...
assert.equal(wave6Choices.filter((choice) => choice.systemChoice).length, 0);
assert.ok(wave6Choices.some((choice) => choice.singleAxisBreakpoint));
...
assert.ok(chassisOnlyChoice.description.includes("잠복"));
assert.ok(!chassisOnlyChoice.description.includes("support bay"));
assert.ok(chassisOnlyChoice.description.includes("Citadel Spindle"));
...
assert.equal(wave6SupportStats, null);
...
assert.equal(supportPayoff?.value, "Bulwark Treads");
assert.equal(game.getSupportBayCapacity(chassisBranchBuild), 1);
...
greedFrameRun.build.architectureForecastId = "storm_artillery";
game.applyForgeChoice(greedFrameRun, greedFrameChoice);
assert.equal(greedFrameRun.build.bastionDoctrineId, "storm_artillery");
assert.equal(greedFrameRun.build.wave6ChassisBreakpoint, true);
...
assert.notEqual(wave7BreakpointRider?.type, "system");
assert.equal(JSON.stringify(game.getVisibleSupportOfferSystemIds(chassisBranchBuild, 7)), JSON.stringify([]));
...
assert.equal(wave8PayoffHeadline?.contractLabel, "주력");
assert.equal(wave8PayoffHeadline?.type, "evolution");
...
assert.equal(wave8PayoffRider?.systemTier, 1);
assert.ok(wave8PayoffRider?.slotText.includes("보조 시스템 설치"));
const wave8PayoffTransform = game.getBaseRouteForgeChoiceTransformation(
  wave8PayoffRider,
  chassisBranchBuild
);
assert.equal(wave8PayoffTransform.previewLabel, "절단 고리");
assert.equal(wave8PayoffTransform.previewValue, "근접 요격 + 외곽 소각");
assert.equal(wave8PayoffTransform.wave8SupportPayoff, null);
...
assert.ok(wave8PayoffMarkup.includes("주력"));
...
assert.equal(shippedLadderWave6[2].label, "ROUTE");
assert.ok(shippedLadderWave6[2].detail.includes("Wave 5"));
assert.ok(shippedLadderWave6[2].detail.includes("Bulwark Treads"));
const shippedRoadmapWave6 = game.getBuildRoadmap(
  chassisBranchBuild,
  game.computeWeaponStats(chassisBranchBuild),
  6
);
assert.ok(shippedRoadmapWave6.steps[1].detail.includes("Bulwark Treads"));
assert.ok(shippedRoadmapWave6.steps[1].detail.includes("몸체 리듬"));
const shippedRoadmapMarkup = game.createShippingLadderMarkup(
  chassisBranchBuild,
  game.computeWeaponStats(chassisBranchBuild),
  6
);
assert.ok(shippedRoadmapMarkup.includes("Wave 5 경로"));
...
assert.equal(supportContractSummary.leadLabel, "Wave 5 경로");
assert.equal(supportContractSummary.leadValue, "Wave 5 진로 선택");
...
assert.equal(supportForgeSummary.leadLabel, "Wave 5 경로");
assert.equal(supportForgeSummary.leadValue, "Wave 5 진로 선택");
...
assert.equal(closingForgeSummary.leadLabel, "Wave 5 경로");
assert.equal(closingForgeSummary.leadValue, "Wave 5 진로 선택");
...
assert.ok(seekerShippedMarkup.includes("Wave 5 경로"));
assert.ok(seekerShippedMarkup.includes("Wave 5 진로 선택"));
...
assert.notEqual(wave7FieldGrantRiderChoice?.type, "system");
...
assert.equal(supportPauseHeroSummary.payoffLabel, "Wave 5 경로");
assert.equal(supportPauseHeroSummary.payoffValue, "Wave 5 진로 선택");
...
assert.equal(wave6HeadlineChoice?.contractLabel, "공세 잠금");
assert.equal(wave6HeadlineChoice?.systemChoice, null);
...
assert.equal(wave6RiderChoice?.contractLabel, "방호 잠금");
assert.equal(wave6RiderChoice?.systemChoice, null);
const wave6GambleChoice = wave6ForgeChoices.find((choice) => choice.contractRole === "gamble");
assert.equal(wave6GambleChoice?.action, "field_greed");
assert.equal(wave6GambleChoice?.contractLabel, "판돈 압박");
...
assert.equal(lateProofSummary.label, "Raptor Sweep");
assert.ok(lateProofSummary.detail.includes("slip lane"));
assert.equal(lateFinaleSummary.label, "Crown Siege");
assert.ok(lateFinaleSummary.detail.includes("외곽"));
...
assert.equal(forgeHeadlineSpotlight.leadValue, "복귀선 하나만 길게 붙든다.");
...
assert.equal(forgeDominantInstallHero, null);
...
assert.equal(forgeFocus.eyebrow, "차체");
...
assert.equal(mirrorHuntWave6Choices.length, 3);
assert.equal(mirrorHuntWave6Choices[0]?.chassisId, "vector_thrusters");
assert.equal(mirrorHuntWave6Choices[1]?.chassisId, "bulwark_treads");
assert.equal(mirrorHuntWave6Choices[2]?.action, "field_greed");
...
assert.equal(kilnBastionWave6Choices[0]?.systemChoice, null);
assert.equal(kilnBastionWave6Choices[1]?.systemChoice, null);
assert.equal(kilnBastionWave6Choices[2]?.action, "field_greed");
assert.equal(kilnBastionWave6Choices[0]?.chassisId, "vector_thrusters");
assert.equal(kilnBastionWave6Choices[1]?.chassisId, "bulwark_treads");
assert.equal(kilnBastionWave6Choices[2]?.chassisId, "salvage_winch");
...
assert.equal(kilnWave6Spotlight.leadValue, "복귀선 하나만 길게 붙든다.");
...
assert.equal(stormArtilleryWave6Choices[0]?.systemChoice, null);
assert.equal(stormArtilleryWave6Choices[1]?.systemChoice, null);
assert.equal(stormArtilleryWave6Choices[2]?.action, "field_greed");
...
assert.equal(shippingLadderWave4.map((step) => step.label).join("|"), "START|THESIS|ROUTE|LAP");
assert.ok(shippingLadderWave4[2].title.includes("Wave 5"));
const shippingLadderFocusWave4 = game.getShippingLadderFocus(roadmapBuild, null, 4);
assert.equal(shippingLadderFocusWave4.label, "ROUTE");
assert.ok(/Wave 5|공세|방호|판돈/.test(shippingLadderFocusWave4.detail));
const shippingLadderWave7 = game.getShippingLadderSteps(seekerProofBuild, null, 7);
const liveRouteStep = shippingLadderWave7.find((step) => step.label === "ROUTE");
assert.equal(liveRouteStep?.title, "Wave 5 진로 선택");
...
const wave6OffenseChoice = recurringWave6Choices.find((choice) => choice.contractRole === "headline");
assert.ok(wave6OffenseChoice);
assert.equal(wave6OffenseChoice.action, "bastion_bay_forge");
assert.equal(wave6OffenseChoice.bayUnlock, false);
assert.equal(wave6OffenseChoice.singleAxisBreakpoint, true);
assert.equal(wave6OffenseChoice.systemChoice, null);
assert.equal(wave6OffenseChoice.contractLabel, "공세 잠금");
const wave6OffenseTransform = game.getBaseRouteForgeChoiceTransformation(wave6OffenseChoice);
assert.equal(wave6OffenseTransform.cardTitle, "Crownsplitter Array");
assert.ok(wave6OffenseTransform.previewLabel.length > 0);
assert.equal(wave6OffenseTransform.previewValue, "Crownsplitter Array");
assert.ok(/Crownsplitter Array|Anvil Prism|split wing|seed|lattice|breach/i.test(wave6OffenseTransform.promise));
assert.ok(/Wave 6-7|corridor|split wing|lattice|breach/i.test(wave6OffenseTransform.proof));
assert.ok(wave6OffenseTransform.accent.includes("Split-Wing Chassis"));
assert.ok(!/proof|support 없이도/i.test(wave6OffenseTransform.proof));
...
assert.equal(wave6DefenseChoice.contractLabel, "방호 잠금");
assert.equal(wave6DefenseChoice.systemChoice, null);
const wave6DefenseTransform = game.getBaseRouteForgeChoiceTransformation(wave6DefenseChoice);
assert.equal(wave6DefenseTransform.cardTitle, "Citadel Spindle");
assert.equal(wave6DefenseTransform.previewValue, "Citadel Spindle");
const wave6GreedChoice = recurringWave6Choices.find((choice) => choice.contractRole === "gamble");
assert.ok(wave6GreedChoice);
assert.equal(wave6GreedChoice.action, "field_greed");
assert.equal(wave6GreedChoice.contractLabel, "판돈 압박");
const wave6GreedTransform = game.getBaseRouteForgeChoiceTransformation(wave6GreedChoice);
assert.equal(wave6GreedTransform.previewValue, "Towchain Reaver");
const wave6HeadlineMarkup = game.createBaseRouteForgeHeadlineCardMarkup({
  choice: wave6OffenseChoice,
  index: 0,
  kind: wave6OffenseChoice.action || wave6OffenseChoice.type || "choice",
  contractLabel: wave6OffenseChoice.contractLabel,
  transformation: wave6OffenseTransform,
  combatAsk: game.getBaseRouteForgeChoiceCombatAsk(wave6OffenseChoice, 6),
  slotLabel: "무료",
  disabled: false,
});
...
assert.ok(!wave6HeadlineMarkup.includes(game.getBaseRouteForgeChoiceTransformation(wave6DefenseChoice).proof));
...
game.applyForgeChoice(
  { build: crownfireBuild, player: null },
  crownfireWave6Choices.find((choice) => choice.contractRole === "gamble")
);
...
pollutedShippingBuild.auxiliaryJunctionLevel = 1;
pollutedShippingBuild.supportBayCap = 3;
...
assert.equal(sanitizedShippingBuild.auxiliaryJunctionLevel, 0);
assert.equal(sanitizedShippingBuild.supportBayCap, 1);
...
assert.equal(pollutedShippingBuild.auxiliaryJunctionLevel, 0);
assert.equal(pollutedShippingBuild.supportBayCap, 1);
...
assert.equal(game.WAVE_CONFIG[11].hazard.type, "drift");
assert.equal(game.WAVE_CONFIG[11].hazard.count, 2);
assert.ok(game.WAVE_CONFIG[11].hazard.driftSpeed > 0);
...
assert.equal(mirrorWaveEleven.label, "Wave 11 · Raptor Sweep");
assert.equal(mirrorWaveTwelve.label, "Wave 12 · Crown Siege");
assert.equal(bastionWaveNine.label, "Wave 9 · Payoff Run+");
assert.equal(bastionWaveTen.label, "Wave 10 · Crown Proof+");
assert.equal(bastionWaveEleven.label, "Wave 11 · Bulwark Rotation");
assert.equal(bastionWaveTwelve.label, "Wave 12 · Citadel Lock");
assert.equal(artilleryWaveNine.label, "Wave 9 · Payoff Run+");
assert.equal(artilleryWaveTen.label, "Wave 10 · Crown Proof+");
assert.equal(artilleryWaveEleven.label, "Wave 11 · Exit Vector");
assert.equal(artilleryWaveTwelve.label, "Wave 12 · Jackpot Lock");
...
assert.equal(mirrorWaveEleven.arena.width, 1980);
assert.equal(mirrorWaveTwelve.arena.height, 1160);
assert.equal(bastionWaveEleven.arena.width, 1940);
assert.equal(bastionWaveTwelve.arena.height, 1140);
assert.equal(artilleryWaveEleven.arena.width, 2000);
assert.equal(artilleryWaveTwelve.arena.height, 1160);
assert.ok(mirrorWaveEleven.arena.width > game.WAVE_CONFIG[7].arena.width);
assert.ok(mirrorWaveTwelve.arena.width > mirrorWaveEleven.arena.width);
assert.ok(bastionWaveEleven.arena.width > game.WAVE_CONFIG[7].arena.width);
assert.ok(bastionWaveTwelve.arena.width > bastionWaveEleven.arena.width);
assert.ok(artilleryWaveEleven.arena.width > game.WAVE_CONFIG[7].arena.width);
assert.ok(artilleryWaveTwelve.arena.width > artilleryWaveEleven.arena.width);
...
assert.ok(mirrorWaveEleven.mix.lancer > mirrorWaveEleven.mix.binder);
assert.ok(mirrorWaveEleven.activeCap <= 27);
assert.ok(mirrorWaveEleven.hazard.count >= 2);
...
assert.ok(artilleryWaveTwelve.activeCap <= 31);
...
assert.equal(biasedWave6Choices[0]?.systemChoice, null);
assert.equal(biasedWave6Choices[1]?.systemChoice, null);
assert.equal(biasedWave6Choices[2]?.action, "field_greed");
...
assert.equal(wave6DoctrineChoices[0]?.action, "bastion_bay_forge");
assert.equal(wave6DoctrineChoices[0]?.bayUnlock, false);
assert.equal(wave6DoctrineChoices[0]?.systemChoice, null);
assert.ok(wave6DoctrineChoices[0]?.singleAxisBreakpoint);
assert.ok(wave6DoctrineChoices[0]?.chassisId);
assert.equal(wave6DoctrineChoices[1]?.action, "bastion_bay_forge");
assert.equal(wave6DoctrineChoices[2]?.action, "field_greed");
...
assert.equal(wave6ChassisPackages[0]?.chassisId, "vector_thrusters");
assert.equal(wave6ChassisPackages[0]?.bayUnlock, false);
assert.equal(wave6ChassisPackages[0]?.systemChoice, null);
assert.ok(wave6ChassisPackages[0]?.singleAxisBreakpoint);
assert.equal(wave6ChassisPackages[0]?.skipNextAdminStop, false);
assert.equal(wave6ChassisPackages[1]?.action, "bastion_bay_forge");
assert.equal(wave6ChassisPackages[2]?.action, "field_greed");
...
const featuredChassisChoice = wave6ChassisPackages.find((choice) => choice.action === "bastion_bay_forge");
const greedChassisChoice = wave6ChassisPackages.find((choice) => choice.action === "field_greed");
assert.ok(featuredChassisChoice);
assert.ok(greedChassisChoice);
assert.match(greedChassisChoice?.description || "", /Towchain Reaver/);
...
chassisRun.build.bastionDoctrineId = "kiln_bastion";
game.applyForgeChoice(chassisRun, featuredChassisChoice);
assert.equal(chassisRun.build.wave6ChassisBreakpoint, true);
assert.equal(chassisRun.build.supportBayCap, 1);
assert.equal(chassisRun.build.supportSystems.length, 0);
assert.equal(chassisRun.build.chassisId, featuredChassisChoice.chassisId);
assert.equal(game.shouldSkipOwnershipAdminStop(chassisRun.build, 9), false);
assert.equal(game.computeSupportSystemStats(chassisRun.build, 7), null);
assert.equal(JSON.stringify(game.getVisibleSupportOfferSystemIds(chassisRun.build, 7)), JSON.stringify([]));
const delayedWave8ForgeChoices = game.buildForgeChoices(chassisRun.build, () => 0, 999, {
  nextWave: 8,
  finalForge: false,
});
const delayedWave8ForgeSupportChoice = delayedWave8ForgeChoices.find((choice) => choice.type === "system");
assert.ok(delayedWave8ForgeSupportChoice);
assert.ok(["kiln_sentry", "aegis_halo", "seeker_array"].includes(delayedWave8ForgeSupportChoice.systemId));
assert.equal(
  JSON.stringify(game.getVisibleSupportOfferSystemIds(chassisRun.build, 8)),
  JSON.stringify(["aegis_halo"])
);
...
const featuredWave6Proof = game.resolveWaveConfig(5, chassisRun.build);
const featuredWave7Proof = game.resolveWaveConfig(6, chassisRun.build);
...
const bulwarkFeaturedChoice = game
  .buildWave6ChassisBreakpointChoices(bulwarkProofBuild, () => 0, 6)
  .find((choice) => choice.chassisId === "bulwark_treads");
game.applyForgeChoice(
  { build: bulwarkProofBuild, resources: { scrap: 0 }, stats: {}, player: null },
  bulwarkFeaturedChoice
);
...
assert.match(bulwarkWave7Proof.directive, /절단 고리|재진입|seam/);
...
const salvageFeaturedChoice = game
  .buildWave6ChassisBreakpointChoices(salvageProofBuild, () => 0, 6)
  .find((choice) => choice.action === "field_greed");
game.applyForgeChoice(
  { build: salvageProofBuild, resources: { scrap: 0 }, stats: {}, player: null },
  salvageFeaturedChoice
);
const salvageWave6Proof = game.resolveWaveConfig(5, salvageProofBuild);
assert.equal(salvageWave6Proof.chassisProof, undefined);
assert.equal(salvageProofBuild.lateFieldConvergenceId, "towchain_reaver");
assert.ok((salvageProofBuild.midrunGreedRouteUntilWave || 0) >= 8);
...
assert.equal(bastionOvercommitChoices[0]?.action, "bastion_bay_forge");
assert.equal(bastionOvercommitChoices[0]?.bayUnlock, false);
assert.equal(bastionOvercommitChoices[0]?.singleAxisBreakpoint, true);
assert.equal(bastionOvercommitChoices[0]?.systemChoice, null);
assert.equal(bastionOvercommitChoices[1]?.action, "bastion_bay_forge");
assert.equal(bastionOvercommitChoices[2]?.action, "field_greed");
...
assert.ok(
  architectureRun.build.upgrades.includes("Chassis Breakpoint: visible support frame installed with the new chassis")
);
assert.equal(game.shouldSkipOwnershipAdminStop(architectureRun.build, 9), false);
assert.equal(game.unlockLateSupportBay(architectureRun.build), false);
assert.equal(game.getSupportBayCapacity(architectureRun.build), 1);
...
assert.equal(game.doctrineAllowsSystemInstall(architectureRun.build, "aegis_halo", 8), false);
...
const artilleryAscensionChoice = game
  ...
  .find(
    (choice) =>
      ...
      choice.chassisId === "bulwark_treads"
  );
...
assert.ok(artilleryAscensionChoice.description.includes("Citadel Spindle"));
assert.equal(artilleryAscensionChoice.systemChoice, null);
...
assert.equal(game.doctrineAllowsSystemInstall(artilleryDoctrineBuild, "aegis_halo", 8), false);
assert.equal(
