...
assert.equal(lateProofSummary.label, "Crownbreaker Lap");
assert.ok(lateProofSummary.detail.includes("전장 폭"));
assert.equal(lateFinaleSummary.label, "Cataclysm Finale");
assert.ok(lateFinaleSummary.detail.includes("최종 breach"));
...
assert.ok(lateRouteFinaleBeat.detail.includes("Cinder Bastion"));
...
assert.equal(crownWindowEscalation.bandId, "cinder_bastion_hold");
...
assert.equal(bastionWaveNine.label, "Wave 9 · Halo Bastion");
assert.equal(bastionWaveTen.label, "Wave 10 · Halo Refuge");
assert.equal(bastionWaveEleven.label, "Wave 11 · Halo Refuge");
assert.equal(bastionWaveTwelve.label, "Wave 12 · Citadel Stand");
assert.equal(artilleryWaveNine.label, "Wave 9 · Ledger Vaultline");
assert.equal(artilleryWaveTen.label, "Wave 10 · Jackpot Caravan");
...
assert.equal(bastionWaveNine.hazard.type, "territory");
assert.equal(artilleryWaveNine.hazard.type, "salvage");
assert.equal(bastionWaveNine.pressureFamily, "territory");
assert.equal(artilleryWaveNine.pressureFamily, "raid");
assert.equal(mirrorWaveTen.hazard.type, "relay");
assert.equal(bastionWaveTen.hazard.type, "drift");
assert.equal(artilleryWaveTen.hazard.type, "caravan");
assert.equal(bastionWaveTen.pressureFamily, "pursuit");
assert.equal(artilleryWaveTen.pressureFamily, "caravan");
...
assert.equal(artilleryWaveTen.arena.width, 1880);
...
assert.ok(bastionWaveNine.mix.warden >= bastionWaveNine.mix.shrike);
assert.ok(artilleryWaveNine.mix.binder > artilleryWaveNine.mix.brander);
...
assert.ok(artilleryWaveTen.hazard.driftSpeed > 0);
...
assert.equal(sharedLateWaveTwelve.label, "Wave 12 · Cinder Bastion");
