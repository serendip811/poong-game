...
assert.ok(game.WAVE_CONFIG[8].mix.mortar > 0);
...
assert.ok(game.WAVE_CONFIG[11].hazard.relayWidth > game.WAVE_CONFIG[9].hazard.relayWidth);
...
assert.equal(bastionWaveNine.label, "Wave 9 · Foundry Hold");
assert.equal(bastionWaveNine.hazard.type, "territory");
assert.ok(bastionWaveNine.hazard.salvageScrap > 0);
assert.ok(bastionWaveNine.activeCap < game.WAVE_CONFIG[8].activeCap);
const bastionWaveTen = game.resolveWaveConfig(9, bastionLateBuild);
assert.equal(bastionWaveTen.label, "Wave 10 · Reclaim Kiln");
assert.equal(bastionWaveTen.hazard.type, "drift");
assert.ok(bastionWaveTen.mix.brute > bastionWaveTen.mix.shrike);
assert.ok(bastionWaveTen.activeCap < game.WAVE_CONFIG[9].activeCap);
const bastionWaveEleven = game.resolveWaveConfig(10, bastionLateBuild);
assert.equal(bastionWaveEleven.label, "Wave 11 · Bastion Furnace");
assert.equal(bastionWaveEleven.hazard.type, "territory");
assert.ok(bastionWaveEleven.hazard.salvageScrap >= bastionWaveNine.hazard.salvageScrap);
const bastionWaveTwelve = game.resolveWaveConfig(11, bastionLateBuild);
assert.equal(bastionWaveTwelve.label, "Wave 12 · Bulwark Crown");
assert.equal(bastionWaveTwelve.hazard.type, "territory");
assert.ok(bastionWaveTwelve.hazard.salvageScrap > bastionWaveEleven.hazard.salvageScrap);
