...
assert.equal(game.WAVE_CONFIG[5].hazard.type || "pulse", "pulse");
...
assert.ok(game.WAVE_CONFIG[7].hazard.count > game.WAVE_CONFIG[6].hazard.count);
...
assert.equal(breaklineFollowthrough.hazard.type || "pulse", "pulse");
...
assert.ok(crownfireSpike.hazard.count < forgecrossSpike.hazard.count);
