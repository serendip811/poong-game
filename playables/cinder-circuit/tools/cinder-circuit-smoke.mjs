...
assert.equal(breaklineFollowthrough.hazard.type, "territory");
assert.ok(afterglowWindow.activeCap < breaklineFollowthrough.activeCap);
assert.ok(afterglowWindow.hazard.count < breaklineFollowthrough.hazard.count);
...
const crownPayoff = game.resolveWaveConfig(10, lateCacheBuild);
const crownEscalation = game.resolveWaveConfig(11, lateCacheBuild);
assert.ok(crownPayoff.activeCap < crownEscalation.activeCap);
assert.ok(crownPayoff.spawnBudget < crownEscalation.spawnBudget);
assert.ok(crownPayoff.hazard.count < crownEscalation.hazard.count);
...
assert.equal(crownWindowPayoff.bandId, "cinder_crown");
assert.equal(crownWindowEscalation.bandId, "cinder_crown");
...
assert.equal(mirrorWaveTen.label, "Wave 10 · Overdrive Gallery");
...
assert.equal(bastionWaveTen.label, "Wave 10 · Halo Bastion Run");
...
assert.equal(mirrorWaveTen.arena.width, 1760);
assert.equal(bastionWaveTen.arena.width, 1660);
assert.equal(artilleryWaveTen.arena.width, 1740);
...
assert.ok(artilleryWaveTen.hazard.salvageScrap > artilleryWaveNine.hazard.salvageScrap);
...
assert.ok(afterburnFour.hazard.salvageScrap > game.WAVE_CONFIG[9].hazard.salvageScrap);
...
assert.equal(afterburnSeven.mix.binder, undefined);
...
assert.equal(game.resolveWaveConfig(8, game.createInitialBuild("relay_oath")).label, "Wave 9 · Lockgrid Hunt I");
