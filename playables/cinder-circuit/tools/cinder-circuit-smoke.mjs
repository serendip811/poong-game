...
assert.equal(
  ...
  JSON.stringify(["Ember Hull", "버티기", "무장 도약", "도약 시험", "주포 폭주", "차체 잠금", "보조 연결", "완성 시험"])
);
...
assert.ok(primedRoadmap.steps[1].detail.includes("Wave 7"));
...
assert.ok(artilleryAscensionChoice.description.includes("Wave 7 포지"));
