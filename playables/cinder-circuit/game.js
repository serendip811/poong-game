(function () {
  ...
  const PRESSURE_ARENA = {
    width: 1160,
    height: 660,
  };
  const FRACTURE_ARENA = {
    width: 1220,
    height: 700,
  };
  const MELTDOWN_ARENA = {
    width: 1260,
    height: 720,
  };
  ...
  const RECURRING_COMBAT_CELL_DEFS = {
    ...
    reset: {
      title: "Pocket Reset",
      ladderStepId: "survive_spike",
      ladderStepLabel: "Pocket Reset",
      pressureFamily: "hold",
      note: (stageText, focusText) =>
        `${stageText} 같은 form으로 finale squeeze 대신 pocket reset을 결산하는 shared hold cell이다. 얇은 refuge pocket 하나만 남겨, ${focusText} 열린 pocket을 지키다 다음 탈출각으로 갈아타는지 마지막으로 확인한다.`,
      directive:
  ...
  const WAVE_CONFIG = [
    {
      ...
      note: "작업장을 조금 넓게 열어 둔 첫 워밍업 구간. 거의 순수한 잔해 무리만 들여보내 기본 회피와 드랍 회수 감각부터 읽히게 만든다.",
      directive: "폭주 지형 없음. 열린 외곽을 길게 돌며 작은 추적 무리만 정리한다.",
      hazard: null,
    },
    {
      id: "pressure",
      ...
      spawnBudget: 44,
      activeCap: 15,
      baseSpawnInterval: 0.94,
      spawnIntervalMin: 0.24,
      spawnAcceleration: 0.3,
      eliteEvery: 11,
      arena: PRESSURE_ARENA,
      mix: {
        scuttler: 0.68,
        brute: 0.32,
        shrike: 0,
      },
      note: "첫 무기 도약 전 마지막 맨선체 시험이다. 느린 전열이 섞이지만 화면 homework는 하나만 남겨 둬 열린 측면에서 진입-이탈 리듬을 익히게 만든다.",
      directive: "폭주 지형 없음. 두꺼운 전열을 먼저 비우고 남는 lane으로 크게 빠져나온다.",
      hazard: null,
    },
    {
      id: "fracture",
      ...
      eliteEvery: 8,
      arena: FRACTURE_ARENA,
      mix: {
        scuttler: 0.44,
        brute: 0.28,
        shrike: 0.28,
      },
      note: "첫 무기 도약을 받기 직전의 판. 엘리트 압박은 오르지만 넓어진 전장 덕분에 새 화선 각을 미리 잡을 수 있다.",
      directive: "빠른 엘리트 순환과 단일 강화 Cinder Surge. 코너에 갇히기보다 열린 lane을 길게 유지한다.",
      hazard: {
    ...
    },
    {
      ...
      spawnBudget: 88,
      activeCap: 22,
      baseSpawnInterval: 0.64,
      spawnIntervalMin: 0.16,
      spawnAcceleration: 0.29,
      eliteEvery: 7,
      arena: MELTDOWN_ARENA,
      mix: {
        scuttler: 0.28,
        brute: 0.28,
        shrike: 0.44,
      },
      note: "첫 전투 구간의 결산이지만 더 이상 압축된 스트레스 테스트는 아니다. 넓어진 작업장에서 단일 폭주 축과 엘리트 돌파를 읽어 Wave 5 포지 욕심으로 이어지게 만든다.",
      directive: "Meltdown Surge. 하나의 굵은 차단선만 피해 열린 사선을 유지하고, Wave 5 Armory 전까지 주포 도약이 먹는 회피 각을 직접 늘린다.",
      driveGainFactor: 1.12,
      hazard: {
    ...
    buildRecurringCombatWave({
      ...
      spawnBudget: 88,
      activeCap: 16,
      baseSpawnInterval: 0.68,
      spawnIntervalMin: 0.172,
      spawnAcceleration: 0.19,
      eliteEvery: 11,
      mix: {
        scuttler: 0.22,
        brute: 0.18,
    ...
    buildRecurringCombatWave({
      ...
      waveNumber: 6,
      cellId: "breach",
      stageText: "Wave 5 payoff 직후의 두 번째 칸은 같은 열린 화면을 corridor hold로 꺾는",
      focusText: "headline midform이",
      duration: 86,
      spawnBudget: 98,
      activeCap: 18,
      baseSpawnInterval: 0.62,
      spawnIntervalMin: 0.16,
      spawnAcceleration: 0.205,
      eliteEvery: 9,
      mix: {
        scuttler: 0.12,
        brute: 0.2,
    ...
    buildRecurringCombatWave({
      ...
      waveNumber: 7,
      cellId: "pursuit",
      stageText: "Wave 7은 막 잠근 gun/body line을 pocket reset cadence로 꺾어 보는",
    ...
    buildRecurringCombatWave({
      ...
      waveNumber: 8,
      cellId: "reset",
      stageText: "Wave 8 결산은 첫 support rider 직전 같은 form을 pocket hold로 마무리하는",
      focusText: "막 잠근 gun/body ownership이",
      duration: 92,
      spawnBudget: 124,
      activeCap: 22,
      baseSpawnInterval: 0.514,
      spawnIntervalMin: 0.142,
      spawnAcceleration: 0.232,
      eliteEvery: 8,
      mix: {
        scuttler: 0.08,
        brute: 0.2,
  ...
  function getBaseRouteWave8ClosureCopy(profileId) {
    ...
    if (profileId === "aegis") {
      return {
        cadenceDetail:
          "Wave 8 완성 시험에서 pocket hold를 얼마나 오래 버티는지 먼저 보고, 끝나면 짧은 승리 랩에서 같은 bastion cadence를 더 오래 굴린다.",
        headlineDetail:
          "Wave 8 완성 시험에서 작은 pocket에 얼마나 깊게 들어갔다가 살아 나오는지 보고, 끝나면 짧은 승리 랩에서 같은 bastion hull을 더 오래 굴린다.",
        roadmapDetail: "Wave 8 완성 시험 -> 짧은 승리 랩",
        proof: "Wave 8 완성 시험에서 plate timing이 버티는 선을 얼마나 오래 여는지 바로 드러난다.",
        statusNote:
          "Wave 8 Warplate Halo가 재충전 warplate 두 겹과 burst pulse를 붙여 몸체를 bastion hull로 바꿨다. 완성 시험에서 plate timing으로 얼마나 깊게 들어갔다가 살아 나오는지 바로 드러나고, 끝나면 짧은 승리 랩에서 같은 hold cadence를 더 오래 굴린다.",
      };
    }
    if (profileId === "ledger") {
      return {
        cadenceDetail:
          "Wave 8 완성 시험에서 얼마까지 더 긁고 빠질지 먼저 가르고, 끝나면 짧은 승리 랩에서 같은 raid cadence를 더 오래 누른다.",
        headlineDetail:
          "Wave 8 완성 시험에서 payout pocket을 얼마나 깊게 긁을지 먼저 가르고, 끝나면 짧은 승리 랩에서 같은 raid frame을 더 오래 몰아본다.",
        roadmapDetail: "Wave 8 완성 시험 -> 짧은 승리 랩",
        proof: "Wave 8 완성 시험에서 더 긁을지 끊고 살아남을지 바로 갈라진다.",
        statusNote:
          "Wave 8 Black Ledger Heist가 주포 양옆에 twin tow fork를 열어 payout lane을 긁는 raid frame으로 바꿨다. 완성 시험에서 더 긁을지 끊고 살아남을지 바로 갈라지고, 끝나면 짧은 승리 랩에서 같은 raid cadence를 더 오래 누른다.",
      };
    }
    return {
      cadenceDetail:
        "Wave 8 완성 시험에서 잠근 실루엣을 바로 증명하고, 끝나면 짧은 승리 랩으로 이번 런을 닫는다.",
      headlineDetail:
        "Wave 8 완성 시험에서 잠근 실루엣을 바로 증명하고, 끝나면 짧은 승리 랩 하나로 sweep 폭을 더 오래 누른다.",
      roadmapDetail: "Wave 8 완성 시험 -> 짧은 승리 랩",
      proof: "Wave 8 완성 시험에서 방금 잠근 실루엣이 실제로 얼마나 오래 전장을 먹는지 바로 본다.",
      statusNote:
        "Wave 8 완성 시험에서 방금 잠근 실루엣을 바로 증명하고, 끝나면 짧은 승리 랩으로 이번 런을 닫는다.",
    };
  }
  ...
  function getLateBreakRoutePreview(choice) {
    ...
    if (CONSOLIDATED_12_WAVE_ROUTE) {
      const closureCopy = getBaseRouteWave8ClosureCopy(choice.lateBreakProfileId);
      return [
        { label: "Wave 8", value: "완성 시험" },
        { label: "그 뒤", value: closureCopy.roadmapDetail.replace("Wave 8 완성 시험 -> ", "") },
      ];
    }
  ...
  function getForgeChoiceTransformation(choice) {
    ...
    if (choice.type === "utility" && choice.action === "field_mutation" && choice.lateBreakProfileId === "mutation") {
      const closureCopy = getBaseRouteWave8ClosureCopy("mutation");
      return {
        laneLabel: choice.forgeLaneLabel || choice.laneLabel || "Forge Lane",
        title: choice.title || choice.slotText || "Unnamed Shift",
        tone: "main",
    ...
    if (choice.type === "utility" && choice.action === "field_aegis" && choice.lateBreakProfileId === "aegis") {
      const closureCopy = getBaseRouteWave8ClosureCopy("aegis");
      return {
        laneLabel: choice.forgeLaneLabel || choice.laneLabel || "Forge Lane",
        title: choice.title || choice.slotText || "Unnamed Shift",
        tone: "defense",
        ...
        proof: CONSOLIDATED_12_WAVE_ROUTE
          ? closureCopy.proof
          : "Wave 9 pocket fight에서 plate timing이 버티는 선을 얼마나 오래 여는지 바로 드러난다.",
        riderLabel: "Support Rider",
    ...
    if (choice.type === "utility" && choice.action === "field_greed" && choice.lateBreakProfileId === "ledger") {
      const closureCopy = getBaseRouteWave8ClosureCopy("ledger");
      return {
        laneLabel: choice.forgeLaneLabel || choice.laneLabel || "Forge Lane",
        title: choice.title || choice.slotText || "Unnamed Shift",
        tone: "greed",
        ...
        proof: CONSOLIDATED_12_WAVE_ROUTE
          ? closureCopy.proof
          : "Wave 9 vaultline에서 더 긁을지 끊고 살아남을지 바로 갈라진다.",
        riderLabel: "Defense / Utility",
  ...
  function getBuildRoadmap(build, weapon = null, waveNumber = 1) {
    ...
    const doctrine = getBastionDoctrineDef(
      (build && (build.bastionDoctrineId || build.architectureForecastId)) || build
    );
  ...
  function createLateBreakAegisChoice(build, nextWave) {
    ...
    return {
      ...
      description:
        CONSOLIDATED_12_WAVE_ROUTE
          ? "재충전식 warplate를 두 겹까지 예열해 큰 한 방을 지우고, plate가 터질 때마다 주변 탄막과 추격선을 같이 뜯어낸다. 이 선택의 headline은 support가 아니라 몸체 자체가 bastion hull로 바뀌는 점이고, 다음 proof는 Wave 8 완성 시험과 이어지는 짧은 승리 랩에서 pocket hold를 얼마나 오래 버티는지다."
      ...
      roadmapDetail: CONSOLIDATED_12_WAVE_ROUTE
        ? getBaseRouteWave8ClosureCopy("aegis").roadmapDetail
        : "Wave 9 bastion pocket -> Wave 10 crownproof -> Wave 11 pursuit run -> Wave 12 final stand",
  ...
  function createLateBreakGreedContractChoice(build, nextWave) {
    ...
    return {
      ...
      description:
        CONSOLIDATED_12_WAVE_ROUTE
          ? "개막 자금을 먼저 당겨 고철과 회수 효율을 크게 올리고, 주포 양옆에 twin tow fork를 열어 payout lane을 긁는 raid frame으로 바꾼다. 대신 Wave 8 완성 시험과 이어지는 짧은 승리 랩에서 더 오래 cash-out할수록 청구서도 같이 커져, greed 판단이 바로 몸의 움직임과 사격 각을 바꾸게 만든다."
      ...
      roadmapDetail: CONSOLIDATED_12_WAVE_ROUTE
        ? getBaseRouteWave8ClosureCopy("ledger").roadmapDetail
        : "Wave 9 vaultline -> Wave 10 crownproof -> Wave 11 pursuit run -> Wave 12 final stand",
  ...
  function applyForgeChoice(run, choice) {
    ...
    if (choice.type === "system") {
      ...
      run.build.previewSupportSystemId = null;
