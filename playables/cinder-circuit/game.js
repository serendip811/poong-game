(function () {
  ...
  const WAVE_CONFIG = [
    ...
    },
    {
      ...
      spawnBudget: 136,
      activeCap: 31,
      baseSpawnInterval: 0.47,
      spawnIntervalMin: 0.138,
      ...
      note: "Act 2 첫 밴드는 규칙을 바로 갈아엎지 않는다. Afterglow와 같은 bastion ecology를 한 번 더 두껍게 반복해, 방금 잠근 body/gun form의 외곽 절개 루트를 실제로 익히게 만든다.",
      directive:
        "anchor crossfire. anchor 수는 아직 적지만 더 오래 남는다. 같은 외곽 회전선 위에서 코어 절개 순서와 엘리트 dive 각을 반복해, form ownership을 밀도로 덮지 않고 배운다.",
    ...
    },
    {
      ...
      note: "Act 2 두 번째 밴드의 시작. 여기서만 mortar 후열이 bastion 뒤에 붙기 시작해, 같은 넓은 lane ownership 싸움에 후열 절단 우선순위 하나만 새로 얹는다.",
      directive:
    ...
    },
    {
      ...
      note: "Act 2 결산이지만 규칙은 Crownfire와 같다. mortar 후열, bastion 코어, 외곽 회전선이 같은 ecology 안에서 조금 더 두꺼워질 뿐이라, 다음 late breakpoint 전 마지막 한 판을 mastery test로 쓰게 만든다.",
      directive:
