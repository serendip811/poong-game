(function () {
  ...
  const WAVE_CONFIG = [
    ...
    },
    {
      ...
      note: "Act 3 staircase의 두 번째 판은 payoff band를 light squeeze로만 연장한다. skimmer-lancer crossfire는 남기되 relay crown은 가장 얇은 회랑 하나만 늦게 죄어, 방금 잠근 late form이 lane ownership를 유지한 채 마지막 한 줄만 직접 찢게 만든다.",
      directive:
        "scrapline hunt. skimmer-lancer crossfire가 이어지지만 가장 얇은 corridor 하나만 늦게 닫힌다. brander와 binder는 보조 견제로만 남고, 먼저 넓은 lane을 먹은 뒤 닫히는 한 줄만 마지막에 찢어 proof 직전까지 dominance를 이어 가야 한다.",
  ...
  const SHARED_LATE_ACT_ENCOUNTER_POOL = {
    8: {
      ...
      pressureFamily: "crossfire",
      note: "Act 3 첫 밴드는 doctrine breakpoint 직후의 deliberate payoff window다. 두 웨이브 중 첫 판은 같은 open-lane hunt를 밀도 낮게 열어, 막 굳은 body/gun form이 측면 sweep와 lancer 돌입을 얼마나 넓게 잘라내는지 먼저 즐기게 만든다.",
      directive:
    ...
    9: {
      ...
      bandFocusId: "doctrine_capstone",
      pressureFamily: "crossfire",
      note: "Lockgrid Hunt 두 번째 판도 같은 ecology를 유지한 채 payoff band를 한 웨이브 더 늘리되, 이번에는 corridor 한 줄만 늦게 죄는 light squeeze를 얹는다. 첫 payoff window에서 찾은 절개 루트를 그대로 유지한 채 마지막 한 줄만 직접 여는 감각을 proof 직전까지 체화하게 만든다.",
      directive:
        "lockgrid hunt. 첫 판보다 pressure clump가 두꺼워졌지만 corridor 하나만 늦게 잠길 뿐 규칙은 거의 같다. skimmer 외곽 sweep를 먼저 접은 뒤 닫히는 한 줄을 언제 찢을지 같은 질문을 한 번 더 푼다.",
  ...
  function renderForgeOverlay() {
    ...
    elements.forgeContext.innerHTML = `
      <article class="forge-focus forge-focus--${riderStep ? "rider" : "headline"} forge-context__card forge-context__card--span-two">
        ...
        <div class="status-list">
          ${createStatusRow(
            useBaseRouteContract ? "Current Form" : "Headline Mutation",
            useBaseRouteContract
              ? dominantFormSummary.label
