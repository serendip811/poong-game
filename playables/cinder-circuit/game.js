(function () {
  ...
  const WAVE_CONFIG = [
    ...
    },
    {
      ...
      note: "Late Breakpoint 직후의 첫 payoff window. 새 late chassis/weapon form을 바로 시험하게 하려고 구조물 세금을 걷어내고, skimmer-lancer crossfire만 남긴 채 밀도를 한 박자 더 늦춘다.",
      directive:
    ...
    },
    {
      ...
      note: "Act 3 staircase의 두 번째 판은 payoff band를 한 웨이브 더 늘리는 rougher hunt다. skimmer-lancer crossfire는 유지하되 얇은 relay crown 하나만 얹어, 방금 잠근 late form이 열린 lane을 더 오래 점유하는지 먼저 즐기게 만든다.",
      directive:
        "scrapline hunt. skimmer-lancer crossfire가 두 번째 판에서 더 두꺼워지고, 늦은 relay crown 하나가 가장 얇은 lane만 살짝 죈다. brander와 binder는 보조 견제로만 남기고, 먼저 sweep 순서를 정한 뒤 열린 lane을 길게 유지해야 한다.",
    ...
    },
    {
      ...
      note: "두 웨이브 payoff band 뒤에 오는 첫 real proof wave. 새로운 endform이 pursuit 덩어리를 얼마나 크게 꺾는지 드러내되, relay upkeep 없이 drift pocket 읽기와 long-turn pathing부터 시험하게 만든다.",
      directive:
  ...
  const SHARED_LATE_ACT_ENCOUNTER_POOL = {
    ...
    9: {
      ...
      bandFocusId: "doctrine_capstone",
      pressureFamily: "crossfire",
      note: "Lockgrid Hunt 두 번째 판도 같은 ecology를 유지한 채 payoff band를 한 웨이브 더 늘린다. 새 규칙은 거의 추가하지 않고 같은 전장을 조금 더 거칠게 반복해, 첫 payoff window에서 찾은 절개 루트를 late-form proof 직전까지 체화하게 만든다.",
      directive:
