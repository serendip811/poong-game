(function () {
  ...
  const WAVE_CONFIG = [
    ...
    },
    {
      ...
      note: "Act 3 두 번째 rung은 같은 crossfire 반복 대신 moving breach split로 꺾는다. contraband caravan이 외곽 arc를 따라 달아나며 얇은 회랑을 스스로 열고 닫아, 방금 잠근 late form으로 greed line을 밟을지 안전한 ownership를 고수할지 직접 가르게 만든다.",
      directive:
        "crownsplit run. smuggler caravan이 외곽 lane을 따라 질주하며 scrap burst를 흘린다. skimmer sweep와 brander pocket이 chase 각을 흔드므로, convoy를 끝까지 물어 큰 payout을 챙길지 중앙 ownership를 남겨 안전하게 다음 proof로 넘길지 매번 직접 정해야 한다.",
      driveGainFactor: 1.42,
      arena: THIRD_ACT_PROOF_ARENA,
      ascensionCarrierType: "binder",
      hazard: {
        label: "Crownsplit Caravan",
        type: "caravan",
        interval: 9.2,
        count: 0,
        radius: 78,
    ...
    },
    {
      ...
      note: "두 웨이브 payoff band 뒤에 오는 첫 real pursuit proof wave다. 이제서야 drift pocket과 ignition pressure가 headline form을 추격선 위에 올려, 늦은 dominance가 실제 추격 덩어리를 얼마나 크게 꺾는지 처음으로 묻는다.",
      directive:
        "starforge ignition pursuit. drifting furnace가 쫓아오고 brander가 현재 pocket을 ignition ring으로 예고한다. 열린 band의 습관을 버리고 곧 폭발할 pocket을 먼저 버리며, locked form으로 pursuit 덩어리 자체를 크게 꺾어야 한다.",
    ...
    },
    {
      ...
      note: "최종 전장은 Act 3에서 처음으로 진짜 full breach tax를 되돌린다. relay crown과 ignition ring이 같은 pocket을 닫고 lancer가 빈 각을 찌르므로, payoff band에서 벌어 둔 lane ownership를 마지막 돌파 창으로 직접 바꿔야 한다.",
      directive:
  ...
  const SHARED_LATE_ACT_ENCOUNTER_POOL = {
    ...
    9: {
      label: "Wave 10 · Crownsplit Run",
      bandId: "crownsplit_run",
      bandLabel: "Crownsplit Run",
      bandFocusId: "doctrine_capstone",
      pressureFamily: "caravan",
      note: "두 번째 판은 corridor relay를 한 번 더 반복하지 않고 moving greed breach로 꺾인다. 열린 gallery에서 벌어 둔 ownership를 유지한 채 달아나는 caravan을 따라 깊게 찢을지, payout을 버리고 중앙 lane을 고정할지 late form의 선택 폭을 바로 드러낸다.",
      directive:
        "crownsplit run. contraband caravan이 외곽 proof lane을 따라 달아나며 큰 scrap burst를 미끼로 던진다. skimmer sweep와 brander pocket이 chase 각을 흔드므로, greed dive로 mobile breach 창을 잡을지 중앙 ownership를 남겨 안전하게 정리할지 즉석에서 정해야 한다.",
    ...
    10: {
      ...
      pressureFamily: "pursuit",
      note: "두 칸짜리 payoff band 뒤의 첫 real proof wave다. drift pocket을 읽는 추격 시험으로 꺾어, 방금 잠긴 chassis/weapon leap가 pursuit hook을 얼마나 크게 꺾는지 바로 증명하게 만든다.",
      directive:
  ...
  function getLateBreakCadenceSummary(profileId) {
    ...
    return {
      ...
      detail:
        "Wave 9는 payoff window, Wave 10은 breach proof, Wave 11은 victory lap, Wave 12는 escalation finale다.",
    };
  }
  ...
  function getBuildRoadmap(build, weapon = null, waveNumber = 1) {
    ...
    let stageThreeDetail = lateBreakHeadline
      ...
      : "Wave 8 Late Break Armory에서 하나의 late-form staircase를 고르면 Wave 9-10 payoff band -> Wave 11 proof -> Wave 12 finale로 고정된다.";
  ...
  function getCombatBandState(build, weapon = null, waveNumber = 1) {
    ...
    if (boundedWave <= 10) {
      ...
      return build && build.doctrineCapstoneId
        ? {
            label: "Breakpoint Run",
            headline: lockedTitle,
            detail:
              "Wave 10은 같은 form을 mobile breach run에 다시 꽂는 증명 판이다. 달아나는 caravan greed line을 끝까지 밟을지, payout을 버리고 중앙 ownership를 고정할지 즉석에서 갈라지므로 방금 잠근 doctrine form이 선택권 자체를 얼마나 넓히는지 확인하게 만든다.",
          }
        : {
            label: "Breakpoint Run",
            headline: capstoneTitle,
            detail:
              "Wave 10은 capstone 직전 mobile breach proof다. open-lane payoff에서 벌어 둔 공간을 caravan chase와 안전한 lane hold 사이 선택으로 바꾸며, marked elite cache를 회수할 마지막 창을 직접 골라야 한다.",
          };
    }
    if (build && build.lateBreakProfileId) {
  ...
  function createLateBreakAegisChoice(build, nextWave) {
    ...
    return {
      ...
      title: "Warplate Halo",
      description:
        "Act 3 진입 전에 재충전식 warplate를 두 겹까지 예열해 큰 한 방을 지우고, plate가 터질 때마다 주변 탄막과 추격선을 같이 뜯어낸다. Wave 9-10은 headline form이 전장을 먼저 먹는 payoff band로 유지되고, Wave 11은 Halo proof에서 방금 붙인 생존층이 pursuit 압박을 얼마나 더 길게 흘리는지 바로 요구한다. Wave 12에서만 Citadel final stand를 버티게 만든다.",
      roadmapDetail: "Wave 9-10 payoff band -> Wave 11 Halo proof -> Wave 12 final stand",
      slotText: `warplate ${getLateFieldAegisMaxCharges({ lateFieldAegisLevel: nextLevel })}충전 · payoff band -> proof -> finale`,
      cost: 26,
      laneLabel: "Defense / Utility",
      forgeLaneLabel: "Defense / Utility",
      lateFieldAegisLevel: nextLevel,
  ...
  function createLateBreakGreedContractChoice(build, nextWave) {
    ...
    return {
      ...
      title: "Black Ledger Heist",
      description:
        "Act 3 개막 자금을 먼저 당겨 고철과 회수 효율을 크게 올린다. 대신 첫 두 rung은 headline form이 전장을 먹는 payoff band로 유지되고, Wave 11은 Jackpot proof에서 greed를 rider 위치에 묶은 채 pursuit 각을 한 판 더 누리게 하며, Wave 12에서만 blackout finale로 큰 payout 압박을 정면으로 받게 만든다.",
      roadmapDetail: "Wave 9-10 payoff band -> Wave 11 Jackpot proof -> Wave 12 blackout finale",
      slotText: "고철 +52 · 회수 +16% · payoff band -> proof -> finale · 2웨이브 Siege Debt",
      cost: 0,
      scrapGain: 52,
  ...
  function enterArchitectureDraft() {
    ...
    pushCombatFeed(
      CONSOLIDATED_12_WAVE_ROUTE
        ? "Form Break 개시. 이번 정지에서는 세 장 중 하나만 골라 주포 실루엣을 먼저 잠그고, 차체는 Wave 6, support spectacle은 Wave 8로 미뤄 lean한 opening chassis를 유지한다."
    ...
    setBanner(CONSOLIDATED_12_WAVE_ROUTE ? "Form Break" : "Architecture Draft", 0.95);
    renderForgeOverlay();
    updateHUD();
  }
      ...
      shouldUseLateFieldCache(nextWave)
        ? CONSOLIDATED_12_WAVE_ROUTE
          ? isArsenalBreakpointWave(nextWave)
            ? "Act 3 Forge 개시. 이번 정지에는 세 장만 뜬다: 화면을 넓게 먹는 main leap, 오래 버티게 하는 rider, 판돈을 키우는 gamble."
            : "Field Break 개시. 세 장 중 하나만 집고 바로 다음 웨이브로 밀어붙인다. 큰 form jump가 먼저, rider와 gamble은 그 뒤다."
          : isArsenalBreakpointWave(nextWave)
  ...
  function enterBastionDraft() {
    ...
    pushCombatFeed(
      CONSOLIDATED_12_WAVE_ROUTE
        ? wave6AscensionDraft
          ? "Form Break 개시. 이번 정지는 Act 2에서 오래 끌 주력 form과 body plan을 고르는 순간이다. support bay와 off-doctrine lane은 아직 열지 않고, core gun/body만 먼저 굳혀 다음 몇 웨이브를 lean하게 증명하게 만든다."
          : "Forge Break 개시. 이번 브레이크는 이미 키운 주력 위에 더 큰 leap 하나를 얹거나, 작은 rider로 숨통을 트이게 하거나, 판돈을 욕심내는 셋 중 하나만 고르게 한다."
        : wave6AscensionDraft
  ...
  function enterCatalystDraft() {
    ...
    pushCombatFeed(
      CONSOLIDATED_12_WAVE_ROUTE
        ? "Act 3 Forge 개시. 여기서 고르는 한 장이 남은 9-12웨이브의 주력 실루엣을 사실상 결정한다. 먼저 main leap를 보고, 곧바로 그 형태가 화면을 얼마나 먹는지 증명하게 만든다."
        : "Catalyst Crucible 개시. 회수한 촉매를 지금 점화해 Act 3 본편을 괴물 형태로 싸울지, 안정화 회로로 남은 bracket을 운영형으로 비틀지 정한다.",
      "CAT"
    );
    setBanner(CONSOLIDATED_12_WAVE_ROUTE ? "Act 3 Forge" : "Catalyst Crucible", 0.95);
    renderForgeOverlay();
    updateHUD();
  }
